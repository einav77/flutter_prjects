...
class Login:
    ...
    def main(self):
        ...
        c = conn.cursor()
        c.execute("UPDATE users SET is_online = 0 WHERE username = ?", ("a",))
        ...
        def submit():
            ...
            if can_register is True:
                ...
                if is_registered is True:
                    if is_user_online(name) is False:
                        is_online(name)
                        self.root.destroy()
                        game_lobby = Lobby(name, users_score)
                        game_lobby.main()
                    else:
        ...
        def is_online(name):
            conn = sqlite3.connect('users_list.db')
            c = conn.cursor()
            c.execute("UPDATE users SET is_online = 1 WHERE username = ?", (name,))
            conn.commit()
            conn.close()
        ...
        def back_to_start():
            self.root.destroy()
            try:
                from registeration.open_window import Open_window
