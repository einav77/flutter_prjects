...
class Game:
    ...
    def draw_board(self, screen, player):
        ...
        label = font.render(("you're " + player.color + " player"), 1, p.Color("red"))
        label_pos = (7, 620)
        screen.blit(label, label_pos)
        ...
        if self.game_over is False:
            ...
            if self.tie is True:
                label3 = font.render("draw!!", 1, p.Color("red"))
            elif player.color == self.winner:
                label3 = font.render("you won ", 1, p.Color("red"))
            else:
                label3 = font.render("you lost ", 1, p.Color("red"))
            label_pos3 = (260, 700)
    ...
    def play_sound(self, curr_move):
        ...
        if curr_move.kind == "move":
            curr_sound = os.path.join(r'../sounds/move-self.mp3')
        else:
            curr_sound = os.path.join(r'../sounds/capture.mp3')
